...
export type PolicyAssessment = {
  ...
  decisionBasis: "no-material-match" | "policy-retrieval" | "multi-signal-rule" | "needs-semantic-review";
};
...
export function retrievePolicies(input:string, topK=4):PolicyMatch[] {
  ...
  }).filter(match=>match.score>=.28).sort((a,b)=>b.score-a.score).slice(0,topK);
}
...
export function assessPolicyRisk(input:string):PolicyAssessment {
  ...
  const fraud=hasAny(q,["lừa","otp","chuyển tiền","wire money","steal password","virement","überweisung","송금","转账"]);
  ...
  if ((impersonation&&fraud)||(sexual&&minor)||(malware&&theft)) return {level:"block",title:t("Bị chặn sau đối chiếu policy","Blocked after policy review"),reason:t("Nhiều tín hiệu độc lập cùng khớp một nhóm rủi ro nghiêm trọng; đây không phải quyết định từ một từ khóa đơn lẻ.","Multiple independent signals match a critical-risk policy; this is not a single-keyword decision."),checks:[t("Không gọi model hoặc công cụ","No model or tool call"),t("Lưu policy ID và lý do để audit","Record policy IDs and rationale for audit"),"0 generation tokens"],locale,confidence:.96,policyVersion:POLICY_VERSION,matches,decisionBasis:"multi-signal-rule"};
  const critical=matches.some(match=>documents.find(doc=>doc.id===match.id)?.risk==="critical"&&match.score>=.45);
  if (critical) return {level:"review",title:t("Cần làm rõ trước khi quyết định","Review required before a decision"),reason:t("Policy RAG tìm thấy rủi ro nghiêm trọng nhưng bằng chứng chưa đủ để tự động chặn. Hãy làm rõ mục đích, đối tượng và quyền sử dụng.","Policy retrieval found a serious risk, but evidence is insufficient for an automatic block. Clarify purpose, subject and authorization."),checks:[t("Không suy đoán ý định","Do not infer intent"),t("Yêu cầu semantic review hoặc người duyệt","Require semantic or human review")],locale,confidence:.58,policyVersion:POLICY_VERSION,matches,decisionBasis:"needs-semantic-review"};
  if(matches.length) return {level:"consent",title:t("Cần xác nhận quyền và phạm vi","Rights and scope confirmation required"),reason:t("Policy RAG tìm thấy nội dung liên quan quyền, dữ liệu hoặc tác động bên ngoài. Việc truy xuất policy không tự kết luận tính hợp pháp.","Policy retrieval found rights, data, or external-action concerns. Retrieval alone does not determine legality."),checks:[t("Xác nhận quyền và mục đích","Confirm rights and purpose"),t("Hiển thị dữ liệu gửi tới provider","Disclose provider-bound data"),t("Xác nhận lại trước side effect","Reconfirm before side effects")],locale,confidence:Math.max(...matches.map(m=>m.score)),policyVersion:POLICY_VERSION,matches,decisionBasis:"policy-retrieval"};
  return {level:"allow",title:t("Chưa phát hiện rủi ro vật chất","No material policy risk detected"),reason:t("Không có policy liên quan đạt ngưỡng truy xuất. Policy của provider và kiểm tra đầu ra vẫn tiếp tục áp dụng.","No relevant policy met the retrieval threshold. Provider policy and output checks still apply."),checks:[t("Không dựa vào từ khóa đơn lẻ","No single-keyword blocking"),t("Tiếp tục kiểm tra theo ngữ cảnh","Continue contextual checks")],locale,confidence:.72,policyVersion:POLICY_VERSION,matches,decisionBasis:"no-material-match"};
}
