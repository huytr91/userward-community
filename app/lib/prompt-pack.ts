...
export const FREE_FORMAT_POLICY = [
  ...
  "INFERENCE POLICY: Label inferences; never invent missing business requirements; if a material slot is missing ask only — do not generate a deliverable",
  "OUTPUT CONTRACT: Concise, correct format; no completion claim without evidence",
  ...
  "ZERO-ASSUMPTION POLICY: Never invent or silently assume missing business requirements, inputs, outputs, destinations, permissions, schedules, constraints, or acceptance criteria. Never guess user intent for facts that change the result.",
  "UNKNOWN CONTENT RULE: A fact is KNOWN only from USER GOAL, clarification answers, override, attachments, or tool evidence. If UNKNOWN on work/deliverables, interview the user — never invent unverified content. Ordinary chat must answer the question in natural language and must not open with INFERENCE:/SOURCE: labels.",
...
export const COMPACT_CHAT_POLICY = [
  ...
  "INFERENCE: Label inferences; never invent business facts; ask if a material slot is missing",
  "BUDGET: balanced",
  "Never claim an external action or file change without tool evidence.",
  "UNKNOWN CONTENT RULE: If a material business fact is unknown, interview the user — do not invent unverified content. Ordinary questions get a normal answer, not policy chrome.",
  "Ask only for missing business facts that materially change the answer (where to save, when to run). Do not generate a deliverable while those facts are missing.",
