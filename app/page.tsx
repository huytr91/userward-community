...
export default function Home() {
  ...
  const tokenStats = useMemo(() => { const measured = entries.filter(entry=>entry.usage?.totalTokens); const total = measured.reduce((sum,entry)=>sum+(entry.usage?.totalTokens||0),0); const input = measured.reduce((sum,entry)=>sum+(entry.usage?.promptTokens||0),0); const output = measured.reduce((sum,entry)=>sum+(entry.usage?.completionTokens||0),0); const top = [...measured].sort((a,b)=>(b.usage?.totalTokens||0)-(a.usage?.totalTokens||0))[0]; return { total, input, output, top, measured: measured.length }; }, [entries]);
