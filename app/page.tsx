...
export default function Home() {
  ...
  const sendMessage = (briefApproval?: unknown) => {
    const briefApproved = briefApproval === true;
    const text = draft.trim();
    if ((!text && !attachments.length) || !provider) { setProvidersOpen(true); return; }
    if (clarification?.needed || !interviewComplete || (projectIntent.active && !projectBriefConfirmed && !briefApproved)) { if (projectIntent.active && !projectBriefConfirmed && !briefApproved) setUploadError("Hãy trả lời câu hỏi bằng cách tích chọn hoặc nhập phương án khác, rồi xác nhận Project Brief trước khi thực hiện."); return; }
    ...
    fetch("/api/providers/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider, apiKey, model: provider==="OpenRouter"&&projectIntent.freeEligible?"openrouter/free":model, executionMode, attachments: binaryAttachments, maxOutputTokens: executionMode === "execute" ? 7000 : 1800, prompt: `${policy}\n\nUSER GOAL:\n${text || "Phân tích các file đính kèm."}${clarifiedScope ? `\n\nPhạm vi đã làm rõ: ${clarifiedScope}` : ""}${interview}${files}${workspaceContext}` }) })
      ...
      .finally(() => { setSending(false); setClarifiedScope(""); setInterviewAnswers({}); setClarificationOverride(""); setAnswerMode(false); setAttachments([]); setTimeout(() => timelineRef.current?.scrollTo({ top: timelineRef.current.scrollHeight, behavior: "smooth" }), 30); });
  };
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      <div className="composer-wrap">
        ...
        {interviewQuestions.length>0&&<div className="interview-card"><div className="interview-head"><span>?</span><div><b>Hoàn thiện brief theo cách dễ nhất</b><small>Tích đáp án nhanh hoặc gõ phương án riêng ngay trong ô chat</small></div><em>{clarificationOverride.trim()?"Đã có phương án riêng":`${interviewQuestions.filter(question=>interviewAnswers[question.id]?.trim()).length}/${interviewQuestions.length}`}</em></div><div className="interview-table">{interviewQuestions.map((question,index)=><div className="interview-row" key={question.id}><span><b>{index+1}. {question.label}</b><small>{question.ask}</small></span><div className="choice-list">{question.options.map(option=>{const selected=question.multi?(interviewAnswers[question.id]||"").split(" · ").includes(option):interviewAnswers[question.id]===option;return <button type="button" className={selected?"selected":""} onClick={()=>setInterviewAnswers(prev=>{if(!question.multi)return {...prev,[question.id]:option};const current=(prev[question.id]||"").split(" · ").filter(Boolean);const next=current.includes(option)?current.filter(item=>item!==option):[...current,option];return {...prev,[question.id]:next.join(" · ")}})} key={option}><span className="tick-box">{selected?"✓":""}</span>{option}</button>})}</div></div>)}</div><div className="override-actions">{clarificationOverride.trim()&&<span><b>Phương án của bạn:</b> {clarificationOverride}</span>}<button type="button" onClick={()=>setAnswerMode(true)}>{clarificationOverride.trim()?"Sửa trong ô chat":"＋ Gõ phương án khác trong ô chat"}</button></div><p className="interview-note">Phương án bạn tự gõ được ưu tiên hơn lựa chọn AI đề xuất.</p></div>}
        ...
        <div className="execution-mode"><div><b>{executionMode === "execute" ? "Thực thi project có xác nhận" : "Chat & phân tích"}</b><span>{executionMode === "execute" ? `Có thể tạo hoặc sửa nhiều file trong ${folderHandle?.name || "folder đã chọn"}` : "Đọc file đính kèm và trả kết quả trong timeline"}</span></div><button disabled={!folderHandle} onClick={()=>setExecutionMode(mode=>mode === "analyze" ? "execute" : "analyze")}>{executionMode === "execute" ? "Chuyển sang Analyze" : folderHandle ? "Bật Execute" : "Kết nối folder để Execute"}</button></div>{pendingPatch&&<div className="patch-preview"><div><b>{pendingPatch.files.length} file chờ duyệt</b><span>{pendingPatch.summary}</span><small>{pendingPatch.files.map(file=>`${file.operation}: ${file.path}`).join(" · ")}</small></div><button onClick={()=>setPendingPatch(null)}>Hủy</button><button className="apply" onClick={applyPendingPatch}>Tạo/cập nhật file</button></div>}{attachments.length>0&&<div className="attachment-list">{attachments.map(file=><span key={file.name}>↗ {file.name} <small>{Math.ceil(file.size/1024)} KB</small><button onClick={()=>setAttachments(prev=>prev.filter(item=>item.name!==file.name))}>×</button></span>)}</div>}{uploadError&&<div className="upload-error">{uploadError}</div>}{answerMode&&<div className="answer-mode-bar"><div><b>Đang nhập phương án khác</b><span>Nội dung này sẽ ưu tiên hơn gợi ý AI.</span></div><button type="button" onClick={()=>setAnswerMode(false)}>Hủy</button></div>}<div className="composer"><textarea aria-label={answerMode?"Nhập phương án khác":"Nhập yêu cầu"} value={answerMode?clarificationOverride:draft} onChange={e=>{if(answerMode)setClarificationOverride(e.target.value);else{setDraft(e.target.value);setClarifiedScope("")}}} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();if(answerMode){if(clarificationOverride.trim())setAnswerMode(false)}else sendMessage()}}} placeholder={answerMode?"Gõ phương án khác hoặc sửa đề xuất của AI...":provider ? "Nêu mục tiêu hoặc đính kèm file để phân tích..." : "Kết nối model trước khi gửi yêu cầu..."}/><div className="composer-actions"><div><select aria-label="Chọn hãng hoặc model" value={provider || ""} disabled><option>{provider ? `${provider} · ${model}` : "Chưa có model"}</option></select><input ref={fileRef} className="file-input" type="file" multiple accept=".txt,.md,.csv,.json,.js,.jsx,.ts,.tsx,.py,.html,.css,.xml,.yaml,.yml,.sql,.log,.pdf,.png,.jpg,.jpeg,.webp,.gif,text/*,application/pdf,image/*" onChange={e=>attachFiles(e.target.files)}/><button onClick={()=>fileRef.current?.click()}>＋ Đính kèm file</button><button className="context-on"><i/> Context tự động</button></div><button className="send" aria-label={answerMode?"Lưu phương án khác":"Gửi"} onClick={()=>{if(answerMode){if(clarificationOverride.trim()){setAnswerMode(false);setUploadError("")}}else sendMessage()}} disabled={answerMode?!clarificationOverride.trim():(!provider || (!draft.trim()&&!attachments.length) || sending || Boolean(clarification?.needed) || !interviewComplete)}><Icon name="send"/></button></div></div><p>Clarification: Auto · PDF/ảnh tối đa 10 MB · Text/code tối đa 5 MB · Execute luôn cần duyệt</p>
      </div>
    </section>
