...
type Entry = { id: string; kind: Kind; title?: string; text: string; time: string; meta?: string; role?: "user" | "ai"; usage?: TokenUsage; receipt?: ExecutionReceipt; brief?: ClearBrief; notice?: string; needsClarify?: boolean; clarifyGoal?: string };
...
function ReceiptLine({ receipt, t, locale }: { receipt: ExecutionReceipt; t: Translator; locale: AppLocale }) {
  ...
  const savingsLabel = receipt.savings
    ? t("receiptSavingsLine", {
        pct: receipt.savings.savedPct.toLocaleString(numberLocale, { maximumFractionDigits: 1 }),
        saved: receipt.savings.savedTokens.toLocaleString(numberLocale),
        packed: receipt.savings.compactTokens.toLocaleString(numberLocale),
        free: receipt.savings.freeTokens.toLocaleString(numberLocale),
      })
    : "";
  return <details className="execution-receipt receipt-line">
    ...
    <div className="receipt-grid"><section><b>AI</b><span>{receipt.model}</span><span>{receipt.tools.join(" · ")}</span></section><section><b>{t("receiptDataSent")}</b>{receipt.dataSent.map(item=><span key={item}>{item}</span>)}</section><section><b>{t("receiptEvidence")}</b>{receipt.evidence.map(item=><span key={item}>{item}</span>)}{!receipt.evidence.length&&<span>—</span>}</section><section><b>{t("receiptCost")}</b><span>{receipt.usage?`${receipt.usage.totalTokens.toLocaleString(numberLocale)} token${receipt.usage.cost?` · $${receipt.usage.cost.toFixed(4)}`:""}`:"—"}</span><span>{receipt.changes.length?receipt.changes.join(" · "):"—"}</span></section>{receipt.savings&&<section><b>{t("receiptSavings")}</b><span>{savingsLabel}</span><span>{t("receiptSavingsHint")}</span></section>}{receipt.briefSummary&&<section><b>{t("receiptBrief")}</b><span>{receipt.briefSummary}</span>{receipt.briefId&&<span>{receipt.briefId}</span>}</section>}</div>
  </details>;
}
...
async function readProviderJson(response: Response, providerName: string, t: Translator, onDelta?: (text: string, etaSeconds: number | null) => void) {
  if (response.ok && response.headers.get("content-type")?.includes("application/x-ndjson")) {
    ...
    const wallDeadline = Date.now() + PROVIDER_STREAMING_WALL_MS;
    const startedAt = Date.now();
    let sawStarted = false;
    while (true) {
      if (Date.now() >= wallDeadline) throw new ProviderRequestError(explainProviderFailure(504, providerName, "timeout", t));
      let done = false;
      let value: Uint8Array | undefined;
      try {
        ({ done, value } = await readStreamLine(reader, STREAM_IDLE_TIMEOUT_MS));
      } catch (error) {
      ...
      for (const line of lines) if (line.trim()) {
        ...
        if(event.delta){
          text+=event.delta;
          const eta = estimateStreamEtaSeconds({ receivedChars: text.length, elapsedMs: Date.now() - startedAt, targetChars: 1800 * 4 });
          onDelta?.(text, eta);
        }
...
export default function Home() {
  ...
  const lastClearBriefRef = useRef<ClearBrief | null>(null);
  ...
  const sendMessage = (_briefApproval?: unknown, directText?: string, replyContext?: string) => {
    if (requestInFlightRef.current) return;
    const composerSend = directText === undefined;
    let text = (directText ?? draft).trim();
    let notesNow = clarificationOverride.trim();
    ...
    if (composerSend && pendingInterview) {
      ...
      notesNow = clarificationOverride.trim();
      if (!interviewSlotsComplete({ questions: briefQuestionList, answers: interviewAnswers, freeTexts: interviewFreeText })) {
        setUploadError(t("briefSlotsIncomplete"));
        setTimeout(() => timelineRef.current?.scrollTo({ top: timelineRef.current.scrollHeight, behavior: "smooth" }), 30);
        return;
      }
      const unclearIds = unclearInterviewSlotIds({ questions: briefQuestionList, answers: interviewAnswers, freeTexts: interviewFreeText });
      if (unclearIds.length) {
        const followUps = buildUnclearFollowUpQuestions({
    ...
    const requestWallMs = providerRoundTripTimeoutMs(executionMode === "analyze") + 5_000;
    const requestTimeoutId = window.setTimeout(() => {
      controller.abort(Object.assign(new Error("The operation was aborted due to timeout"), { name: "TimeoutError" }));
    }, requestWallMs);
      ...
      .then(data => {
        ...
        if (executionMode === "execute") {
          try {
            ...
            const receipt = createExecutionReceipt({ status: "preview", model: data.selectedModel || selectedModel, context: contextPack, usage, changes: patch.files.map(file => `${file.operation}: ${file.path}`), evidence: ["Provider returned a validated patch preview", "No file has been written"], savings: requestSavings, briefId: runBrief?.id, briefSummary: runBrief ? summarizeClearBrief(runBrief) : undefined });
