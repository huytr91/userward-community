...
function assessLegalRisk(input: string): LegalAssessment {
  const q = input.toLowerCase().trim();
  const impersonationFraud = /(?:giả|nhái|clone|mạo danh).{0,24}(?:giọng|khuôn mặt|sếp|người thân|người nổi tiếng|chính trị gia)|deepfake|lừa.{0,20}(?:chuyển tiền|mật khẩu|otp)|giả bằng chứng/i.test(q);
  const illegalHarm = /(?:hướng dẫn|cách|script).{0,25}(?:hack|ransomware|đánh cắp|chiếm đoạt|phishing|lừa đảo|né xác minh)|(?:phát tán|tạo).{0,20}(?:mã độc|malware)/i.test(q);
  const intimateOrMinor = /(?:ảnh|video|deepfake).{0,20}(?:khoả thân|nhạy cảm|tình dục).{0,20}(?:người thật|trẻ em|vị thành niên)|(?:trẻ em|vị thành niên).{0,20}(?:khoả thân|tình dục)/i.test(q);
  if (impersonationFraud || illegalHarm || intimateOrMinor) return { level: "block", title: "Bị chặn bởi Legal & Safety Gate", reason: "Yêu cầu có dấu hiệu mạo danh/lừa đảo, gây hại trái pháp luật hoặc xâm phạm nghiêm trọng quyền của người khác. App sẽ không gọi model hay công cụ.", checks: ["Không tạo đầu ra", "Không gửi dữ liệu sang provider", "0 token cho bước generation"] };
  const realPersonMedia = /(?:giọng|voice|khuôn mặt|hình ảnh|video|avatar).{0,25}(?:của tôi|người thật|nhân viên|khách hàng|người nổi tiếng)|(?:clone|nhái).{0,15}(?:giọng|mặt)/i.test(q);
  const copyrighted = /(?:dùng|copy|sao chép|lấy nguyên).{0,25}(?:bài hát|phim|video|ảnh|nhạc|logo|nhân vật)|xóa watermark/i.test(q);
  const personalData = /(?:danh sách|dữ liệu|file).{0,20}(?:khách hàng|nhân viên|email|số điện thoại|căn cước)|sinh trắc học/i.test(q);
  const externalEffect = /(?:gửi|đăng|publish|upload|xóa|ghi đè|chuyển tiền|đặt lịch).{0,30}(?:email|tin nhắn|file|video|website|tài khoản|dữ liệu)?/i.test(q);
  if (realPersonMedia || copyrighted || personalData || externalEffect) return { level: "consent", title: "Cần xác nhận quyền trước khi thực hiện", reason: "Yêu cầu có thể liên quan dữ liệu cá nhân, quyền hình ảnh/giọng nói, bản quyền hoặc hành động tạo hậu quả bên ngoài.", checks: ["Xác nhận quyền sử dụng dữ liệu/tài sản", "Hiển thị provider và dữ liệu rời thiết bị", "Gắn nhãn AI/provenance khi áp dụng", "Xác nhận lại trước hành động bên ngoài"] };
  return { level: "allow", title: "Legal Gate: chưa phát hiện rủi ro cần chặn", reason: "Kiểm tra tự động không phát hiện dấu hiệu mạo danh, dữ liệu cá nhân, bản quyền hoặc hành động bên ngoài trong yêu cầu hiện tại.", checks: ["Tiếp tục áp dụng policy provider", "Đầu ra vẫn phải có evidence nếu báo hoàn thành"] };
}
...
export default function Home() {
  ...
  const sendMessage = (briefApproval?: unknown, directText?: string, replyContext?: string) => {
    ...
    const legalManifest = `LEGAL & SAFETY MANIFEST:\nASSESSMENT: ${requestLegal.level.toUpperCase()} — ${requestLegal.reason}\nRULES: Do not facilitate illegal impersonation, fraud, unauthorized intimate content, exploitation of minors, malware/phishing, or evasion of law. For personal data, real-person likeness/voice, copyrighted assets, and external side effects, require valid rights/consent and explicit approval. Apply AI disclosure/provenance when synthetic media may be mistaken for authentic content. A local risk assessment is a safety gate, not legal advice; when legality remains uncertain, stop and request human/legal review.`;
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      <div className="composer-wrap">
        ...
        {draft.trim()&&<div className={`legal-card ${legalAssessment.level}`}><div><span>{legalAssessment.level==="allow"?"✓":legalAssessment.level==="consent"?"⚖":"×"}</span><b>{legalAssessment.title}</b><em>{legalAssessment.level==="allow"?"Cho phép":legalAssessment.level==="consent"?"Cần xác nhận":"Đã chặn"}</em></div><p>{legalAssessment.reason}</p><ul>{legalAssessment.checks.map(check=><li key={check}>{check}</li>)}</ul>{legalAssessment.level==="consent"&&<label><input type="checkbox" checked={legalConsent} onChange={e=>setLegalConsent(e.target.checked)}/><span>Tôi xác nhận có quyền hợp pháp với dữ liệu/tài sản này và đồng ý cho provider đã chọn xử lý trong phạm vi yêu cầu.</span></label>}</div>}
