...
export default function Home() {
  ...
  const sendMessage = () => {
    ...
    if (clarification?.needed || interviewQuestions.some(question => !interviewAnswers[question.id]?.trim()) || (projectIntent.active && !projectBriefConfirmed)) { if (projectIntent.active && !projectBriefConfirmed) setUploadError("Hãy trả lời các câu hỏi và xác nhận Project Brief/ngân sách trước khi thực hiện."); return; }
    ...
    fetch("/api/providers/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider, apiKey, model, executionMode, attachments: binaryAttachments, maxOutputTokens: executionMode === "execute" ? 7000 : 1800, prompt: `${policy}\n\nUSER GOAL:\n${text || "Phân tích các file đính kèm."}${clarifiedScope ? `\n\nPhạm vi đã làm rõ: ${clarifiedScope}` : ""}${interview}${files}${workspaceContext}` }) })
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      <div className="composer-wrap">
        ...
        {interviewQuestions.length>0&&<div className="interview-card"><div className="interview-head"><span>?</span><div><b>Chọn nhanh để app hiểu đúng công việc</b><small>Không cần biết code hay thư viện · Chọn phương án gần đúng nhất</small></div><em>{interviewQuestions.filter(question=>interviewAnswers[question.id]?.trim()).length}/{interviewQuestions.length}</em></div><div className="interview-table">{interviewQuestions.map((question,index)=><div className="interview-row" key={question.id}><span><b>{index+1}. {question.label}</b><small>{question.ask}</small></span><div className="choice-list">{question.options.map(option=><button type="button" className={interviewAnswers[question.id]===option?"selected":""} onClick={()=>setInterviewAnswers(prev=>({...prev,[question.id]:option}))} key={option}>{interviewAnswers[question.id]===option?"✓ ":""}{option}</button>)}<input value={question.options.includes(interviewAnswers[question.id])?"":interviewAnswers[question.id]||""} onChange={e=>setInterviewAnswers(prev=>({...prev,[question.id]:e.target.value}))} placeholder="Yêu cầu khác (nếu có)..."/></div></div>)}</div><p className="interview-note">App sẽ tự chọn công nghệ phù hợp. Chỉ hỏi kỹ thuật khi bạn chủ động yêu cầu.</p></div>}
        {projectIntent.active&&<div className="project-advisor"><div className="advisor-head"><span>PROJECT ADVISOR</span><b>{projectIntent.type}</b><em>{projectBriefConfirmed?"Đã xác nhận":"Chờ xác nhận BRD"}</em></div><div className="advisor-brief"><small>MỤC TIÊU USER</small><p>{draft}</p><small>CHIẾN LƯỢC AI ĐỀ XUẤT</small><p>{projectIntent.strategy}</p></div><div className="budget-options"><button className={budgetTier==="economy"?"active":""} onClick={()=>{setBudgetTier("economy");setProjectBriefConfirmed(false)}}><b>Tiết kiệm</b><span>Ít biến thể · giới hạn retry</span></button><button className={budgetTier==="balanced"?"active":""} onClick={()=>{setBudgetTier("balanced");setProjectBriefConfirmed(false)}}><b>Cân bằng</b><span>Model mạnh ở bước quan trọng</span></button><button className={budgetTier==="quality"?"active":""} onClick={()=>{setBudgetTier("quality");setProjectBriefConfirmed(false)}}><b>Chất lượng</b><span>Nhiều vòng kiểm tra hơn</span></button></div><button className="confirm-brief" disabled={interviewQuestions.some(question=>!interviewAnswers[question.id]?.trim())} onClick={()=>setProjectBriefConfirmed(true)}>{projectBriefConfirmed?"✓ Brief đã xác nhận":"Xác nhận brief & ngân sách"}</button></div>}
    ...
    <aside className="brain"><div className="brain-title"><Icon name="brain"/><div><span>BỘ NHỚ DỰ ÁN</span><b>Đang đồng bộ</b></div><i/></div><div className="brain-tabs"><button className="active">Hiện tại</button><button>Bộ nhớ</button><button>Sử dụng</button></div>
      ...
      <section className="finance-summary"><label>NGÂN SÁCH AI</label><div><span>Chế độ</span><b>{budgetTier==="economy"?"Tiết kiệm":budgetTier==="quality"?"Chất lượng":"Cân bằng"}</b></div><div><span>Chi phí provider đã ghi nhận</span><b>{tokenStats.cost>0?`$${tokenStats.cost.toFixed(4)}`:"Chưa có dữ liệu"}</b></div><small>Minimum chỉ hiển thị chi phí thật provider trả về; không tự ước lượng thành số giả.</small></section>
