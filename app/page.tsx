...
export default function Home() {
  ...
  const sendMessage = (briefApproval?: unknown) => {
    ...
    if (clarification?.needed || interviewQuestions.some(question => !interviewAnswers[question.id]?.trim()) || (projectIntent.active && !projectBriefConfirmed && !briefApproved)) { if (projectIntent.active && !projectBriefConfirmed && !briefApproved) setUploadError("Hãy trả lời các câu hỏi và xác nhận Project Brief/ngân sách trước khi thực hiện."); return; }
