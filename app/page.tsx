...
export default function Home() {
  ...
  const interviewQuestions = useMemo(() => {
    ...
    const questions = [];
    if (!/gmail|outlook|imap|email nguồn/.test(q)) questions.push({ id: "source", label: "Email lấy từ đâu?", ask: "Chọn dịch vụ bạn đang sử dụng.", options: ["Gmail", "Outlook", "Email công ty", "Không rõ"] });
    if (!/folder|thư mục/.test(q) || !/[a-z]:\\|onedrive|google drive|sharepoint/i.test(draft)) questions.push({ id: "destination", label: "Muốn lưu ở đâu?", ask: "Chọn nơi bạn muốn nhìn thấy email sau khi copy.", options: ["Folder trên máy", "Google Drive", "OneDrive", "Chưa quyết định"] });
    if (!/mỗi |hàng |khi có|theo giờ|ngày|tuần/.test(q)) questions.push({ id: "schedule", label: "Khi nào cần chạy?", ask: "Bạn không cần biết cách lập lịch kỹ thuật.", options: ["Khi tôi bấm chạy", "Khi có email mới", "Mỗi ngày", "Mỗi tuần"] });
    if (!/pdf|đính kèm|nội dung|tiêu đề|người gửi|toàn bộ email/.test(q)) questions.push({ id: "scope", label: "Cần copy những gì?", ask: "Chọn phương án gần nhất với kết quả bạn muốn.", options: ["Toàn bộ email", "Chỉ file đính kèm", "Nội dung + file", "Chưa rõ"] });
    return questions;
  }, [draft]);
  ...
  const sendMessage = () => {
    ...
    const policy = `${executionPolicy}\nUSAGE PROFILE: ${usagePlan.profile}\nPRIORITY: ${usagePlan.priority}\nTOOL STRATEGY: ${usagePlan.tool}\nVERIFICATION: ${usagePlan.verification}\nSOURCE POLICY: ${usagePlan.source}\nINFERENCE POLICY: ${usagePlan.inference}\nOUTPUT CONTRACT: ${usagePlan.output}\nEND-USER CLARIFICATION POLICY: Never ask the user to choose libraries, frameworks, APIs, architecture, test tools, or implementation details unless the user explicitly requests technical control. Infer safe technical defaults yourself. If business information is still missing, ask only about desired process, inputs, outputs, frequency, destination, constraints, and acceptance criteria, using plain nontechnical language and no more than 4 concise questions.`;
    let redactions = 0;
